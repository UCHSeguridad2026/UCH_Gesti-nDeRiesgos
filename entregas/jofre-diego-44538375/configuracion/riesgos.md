...
Los riesgos del escenario fueron evaluados con el método `Classic Risk Scoring` configurado en la instancia de SimpleRisk utilizada para el trabajo.

La herramienta muestra el resultado normalizado mediante la siguiente fórmula:

```text
RISK = (Likelihood x Impact) x (10 / 25)
```

Los niveles indicados a continuación corresponden a los valores resultantes observados en SimpleRisk.
...
Se considera una probabilidad media debido a la existencia de distintos vectores potenciales de entrada asociados al uso cotidiano de estaciones de trabajo y sistemas digitales, como phishing, archivos maliciosos o explotación de vulnerabilidades.
...
**Nivel de riesgo resultante:** 6.0 - Medium
...
**Nivel de riesgo resultante:** 4.8 - Medium
...
**Nivel de riesgo resultante:** 6.4 - Medium
...
**Nivel de riesgo resultante:** 3.6 - Medium
...
# Planes de acción
Como parte del tratamiento de los riesgos identificados, se definieron tres planes de acción asociados a los riesgos priorizados por su puntuación e impacto.
...
**Nota del autor:**
### Consideración sobre los niveles de riesgo

Durante la evaluación se observó que la instancia de SimpleRisk utilizada normaliza el resultado del método Classic mediante la fórmula:

`RISK = (Likelihood x Impact) x (10 / 25)`

Como consecuencia, riesgos con combinaciones elevadas de probabilidad e impacto, por ejemplo 4/5 y 4/5 o 3/5 y 5/5, son clasificados por la herramienta dentro del nivel `Medium`.

Por este motivo, para la selección de los planes de acción no se consideró únicamente la etiqueta de nivel asignada por SimpleRisk, sino también la puntuación obtenida, el impacto potencial y la importancia de los activos afectados.

De esta manera se priorizaron los riesgos de phishing, ransomware y pérdida o corrupción de backups, al considerarse los más relevantes para la continuidad y seguridad de la clínica dentro del escenario analizado